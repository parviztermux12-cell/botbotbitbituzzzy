...
TOKEN = "8746694158:AAFkiKlfdKVykPJwY-xCnJaco_8eRsg8UmQ"
...
START_BALANCE = 25000
PROMO_CHAT_ID = -1003788516764  # ID чата для отправки промокодов (None - отключено)
...
def top_cmd(message):
    ...
    # Отправляем топ-50
    send_top_page(message, users, limit)

def send_top_page(message, users, limit):
    """Отправляет сообщение с топом"""
    
    # Берем первые limit пользователей
...
@bot.message_handler(func=lambda m: m.text and m.text.lower() in ["помощь", "/help@izzzy_game_bot", "/help"])
def cmd_help(message):
    user_id = message.from_user.id
...
REQUIRED_BIO_TEXT = "@izzzy_game_bot Лучший игровой чат-бот. Присоединяйся!"
...
def my_tyanka(message):
    ...
    text = (
        ...
        f"🛠 <a href='https://t.me/izzzy_vpn'>Канал разработчика</a>"
    )
...
STAR_CHAT_ID = -1003788516764  # ID чата для подсчёта сообщений
STARS_PER_MESSAGE = 0.0010  # 0.01 звезды за 2 сообщения = 0.005 за 1 сообщение
